# CLAUDE.md — Daxa POS Build Instructions
...
Claude Code may assist with planning, implementation, documentation, testing, review, and maintenance, but the system must be built under human supervision.

Claude Code must not make uncontrolled architectural decisions, skip documentation, silently change agreed direction, or treat temporary implementation convenience as product direction.
...
Claude Code must follow these rules every time it runs.

### Autonomous planning behaviour

When Claude Code is asked to plan, triage, investigate, or prepare fixes, it must work autonomously from the repository contents.

Claude Code must not ask the human for file paths, issue details, commands, or project structure that can be discovered by reading the repository.

Claude Code must proceed with reasonable assumptions when the repository provides enough context.

Assumptions must be written into the active plan or worker notes.

Claude Code may only stop for human input when:
...
For every planning or triage session, Claude Code must create or update:
...
Claude Code must leave enough notes that a later worker can continue without re-discovering the same information.
...
If any of these files do not exist, Claude Code must create a documentation issue or include the missing file in the plan.

---

## Planning Before Work

Before making changes, Claude Code must create or update a short implementation plan.
...
Claude Code must not make more than three meaningful changes without pausing and updating the active plan.
...
Claude Code must commit each completed logical change.
...
Claude Code must keep `./docs` current.
...
Claude Code must add or update tests for behaviour it changes.
...
The preferred workflow is to use specialised Claude Code workers sequentially.
...
Claude Code should integrate with GitHub for repository tracking where available.
...
Claude Code must clearly identify:
...
Claude Code must not claim production readiness unless the relevant tests, deployment checks, security checks, and documentation are complete.
...
Claude Code should maintain a docs structure similar to:
