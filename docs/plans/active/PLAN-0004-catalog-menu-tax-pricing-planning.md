...
**Status: Done (2026-07-05).** Implemented per the plan's exact permission table — `pricing.manage` (not `catalog.manage`) for `ProductLocationOverride`/`VenueTaxConfiguration`, `catalog.sold-out-toggle` + `rejectStaffPin: false` for the sold-out toggle — confirmed against this section before writing any endpoint. `PriceResolver` was TDD'd first (financial logic, mandatory per CLAUDE.md). One addition beyond the plan's prose: the sold-out toggle also checks a location-bound staff-PIN session's own `LocationId` against the target, not just organisation membership. See `PLAN-0004-worker-notes.md`'s Milestone F Report for full detail and deviations.
...
**Status: Done (2026-07-05).** Test-and-documentation-only as scoped — no new entities, migrations, or endpoint groups. `RbacTests.cs`'s `PermissionGatedEndpoints()` extended with the 73 PLAN-0004 endpoints across Milestones C–G (401/403/device-token/staff-PIN matrix); the resolved-menu read added to `AllProtectedEndpoints()` for its 401 sweep only (its staff-succeeds/no-permission-required behaviour stays in the dedicated `ResolvedMenuEndpointsTests`, not duplicated). `IgnoreQueryFiltersUsageTests.cs` re-verified passing with the unchanged 5-file allowlist — zero new PLAN-0004 call sites. All 18 PLAN-0004 endpoint groups confirmed registered in `Program.cs` (already complete from Milestones C–G, no fix needed). Permission categories and role grants re-verified against `RolePermissionConfiguration.cs`/the existing `PermissionCatalogue_ClassifiesPLAN0004MilestoneAPermissions_ByCategory` test — both already correct, no changes needed. ADR-0016 accepted and moved to `docs/adr/accepted/` (see the Milestone H ADR-0016 acceptance note above). OI-0017 opened for the archive-and-replace concurrency race; the other two candidate issues (`VenueTaxConfiguration`-absence, menu merge-precedence revisit) were evaluated and found already resolved/not warranted — see `PLAN-0004-worker-notes.md`'s Milestone H Report for the full reasoning.
...
- **ADR-0016 — Accepted 2026-07-05, as part of Milestone H closeout.** Verified internally consistent, verified nothing in Milestones A–G contradicts it, and confirmed acceptance requires no schema/code work — only formalizes the constraints this plan already honoured throughout. See the ADR-0016 check note under Milestone H below and `PLAN-0004-worker-notes.md`'s Milestone H Report.
...
**Milestone H ADR-0016 acceptance (2026-07-05):** per explicit human approval granted for this closeout milestone, moved to `docs/adr/accepted/ADR-0016-multi-language-and-localisation-strategy.md`. Before moving: read the ADR in full (internally consistent — no contradictions found across its 9 sections, Consequences, Alternatives, and Follow-Up Work); confirmed Milestones A–G implemented nothing that contradicts it (no `{Entity}Translation` tables built, no business logic string-matches on any translatable `Name`/label field — matching is always on `Id`/`Code`, e.g. `TaxDefinitionTemplate.Code`/`TaxCategory.Code`/`Product.Sku`, confirmed by re-reading every milestone's ADR-0016 check note above — and every `AuditEvent` write uses a stable `EventType` code plus JSON `BeforeValue`/`AfterValue` snapshots, never a pre-rendered sentence, per §6); confirmed acceptance requires no schema/code work in this milestone (the ADR's own text: "no implementation required by acceptance, only formalizes constraints this plan already honours"). Updated `docs/adr/index.md` (moved the row, "Proposed" section now empty), the ADR's own Status field, its internal sibling-ADR relative links (now same-directory, no `../accepted/` prefix needed), and every living doc that referenced the old `proposed/` path (`docs/README.md`, `docs/03-phase-roadmap.md`, `docs/architecture/overview.md`, `docs/architecture/tax-engine.md`, `docs/modules/tax.md`, `docs/modules/catalog.md`, `docs/modules/receipts.md`, `docs/plans/active/PLAN-0006-terminal-display-pwa-planning.md`, `docs/plans/active/PLAN-localisation-multi-language.md`) — historical point-in-time records in `PLAN-0003-worker-notes.md`, `PLAN-0003-identity-tenancy-locations-devices.md`, and this plan's own per-milestone check notes above were deliberately left unchanged, since rewriting them would falsify what was actually true at each of those past points in time.
...
6. **Accept ADR-0016 now** (as Accepted, not just Proposed) — no implementation follows from acceptance, only formalizes constraints this plan already honours. Recommendation: yes, accept now, since PLAN-0004 is exactly the "first implementing plan" ADR-0016's own Follow-Up Work section anticipated. **Actioned 2026-07-05, Milestone H** — see the Milestone H ADR-0016 acceptance note above.
