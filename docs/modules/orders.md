...
- Order state machine: `Open ⇄ Held` (park/resume); any line may be voided (reversal, never deleted) while `Open`/`Held`; the whole order may become `Voided`/`Cancelled` from `Open`/`Held` (no restriction on line count — a fully-built order can still be cancelled). `Completed` is now reachable (PLAN-0005 Milestone B): recording a payment that fully settles the order's `GrandTotalAmount` transitions `Status` to `Completed` and sets `ClosedAtUtc` — see `docs/modules/payments.md`.
