...
Six staff audit event types added: `StaffMemberCreated`, `StaffMemberPinReset`, `StaffMemberRoleAssigned` (one `StaffMemberLifecycleDomainEvent` carrying an `Action` field, per the Milestone D convention), `StaffPinLoginSucceeded`, `StaffPinLoginFailed`, `StaffMemberDisabled`. Unlike unknown emails (Milestone C) and unknown registration PINs (Milestone E), **unknown staff-code login attempts are audited** (`Reason = "UnknownStaffCode"`, `StaffMemberId` null): the trusted device context supplies the tenant, so the non-nullable `AuditEvent.TenantId` is always satisfiable on this path — the client still only ever sees a generic failure. Other audited failure reasons: `LocationMismatch`, `StaffInactive`, `LockedOut`, `HomeLocationMismatch`, `InvalidPin`, `RoleGrantsSensitivePermissions` (a role-configuration error, also logged at error level).

## Implementation status (PLAN-0003 Milestone H closeout, 2026-07-03)

PLAN-0003 is complete for the audit foundation: `AuditEvent` plus in-process domain-event handlers cover every login/session/device/staff lifecycle action this plan introduced, appended-only, `jsonb` before/after snapshots. The recorded tenant-less gap — unauthenticated attempts against an unknown identifier (unknown email, unknown registration PIN) cannot be audited while `AuditEvent.TenantId` stays non-nullable — remains open as [OI-0014](../issues/open/OI-0014-tenantless-security-event-auditing.md), deliberately not solved here; rate limiting and infrastructure logs cover that gap in the interim. There is still no audit read/reporting API (`docs/testing/local-smoke-test.md` verifies audit rows via direct `psql` queries) — later plan's responsibility.
