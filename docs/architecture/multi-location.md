...
### PLAN-0004 Milestone F implementation status (2026-07-05)

Location-level catalog overrides ("Product availability" and "Prices" from the Location-Level Features list above) are implemented: `ProductLocationOverride` (one row per `(Product, Location)` pair; absence means "use the organisation-wide default") and `VenueTaxConfiguration` ("Tax profile," one row per location; absence 404s rather than silently defaulting — the plan's approved Human Decision #5). The sold-out toggle (`POST /api/v1/products/{productId}/locations/{locationId}/sold-out`) is the first endpoint in the system where a staff-PIN session's own location genuinely restricts its reach: a session bound to Location A's registered device cannot toggle sold-out at Location B, even within the same organisation — an org-scoped admin session (no location binding) has no such restriction. See [Module: Catalog](../modules/catalog.md) and [Module: Pricing](../modules/pricing.md) for full detail.
